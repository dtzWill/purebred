...
import System.IO (openTempFile, hClose, withBinaryFile, IOMode(..))
import System.Directory (getTemporaryDirectory, removeFile)
import System.Process.Typed (proc, setStdin, useHandleClose)
...
openCommand' s cmd
  ...
  | otherwise = liftIO $ do
      ...
      f <- getTempFileForEditing maybeEntity
      tryRunProcess (proc cmd [f]) >>= either (handleIOException s) (pure . handleExitCode s)
...
pipeCommand' s cmd
  ...
  | otherwise = liftIO $ do
      ...
      withBinaryFile f ReadMode $ \h ->
        tryRunProcess (setStdin (useHandleClose h) $ proc cmd [])
